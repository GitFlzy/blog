...
function onUpdateReady() {
    com.zybuluo.common.popupConfirm.popup("123鏂扮増鏈€氱煡", '鍙戠幇鏂扮増鏈紝 鍦ㄨ繖閲屾煡鐪�<a href="https://www.zybuluo.com/ghosert/note/102771" target="_blank"> <i class="icon-wrench"></i> 鏇存柊璇︾粏</a>锛岀偣鍑讳笅鏂规寜閽繘琛屾洿鏂版搷浣溿€�', function () {
        window.location.reload()
    }, function () {
        window.location.reload()
    }, function () {
        $("#notification-popup-window #notification-confirm").html("456鏇存柊"), $("#notification-popup-window button.close").hide(), $("#notification-popup-window #notification-cancel").hide()
    }, function () {
        window.location.reload()
    })
}! function (a, b) {
    ...
    window.PR_SHOULD_USE_CONTINUATION = !0,
        function () {
            ...
            function g(b) {
                ...
                return (e = b.types) && d.push(["typ", e]), e = ("" + b.keywords).replace(/^ | $/g, ""), e.length && d.push(["kwd", RegExp("^(?:" + e.replace(/[\s,]+/g, "|") + ")\\b"), a]), c.push(["pln", /^\s+/, a, " \r\n	聽"]), e = "^.[^\\s\\w.$@'\"`/\\\\]*", b.regexLiterals && (e += "(?!s*/)"), d.push(["lit", /^@[$_a-z][\w$@]*/i, a], ["typ", /^(?:[@_]?[A-Z]+[a-z][\w$@]*|\w+_t\b)/, a], ["pln", /^[$_a-z][\w$@]*/i, a], ["lit", /^(?:0x[\da-f]+|(?:\d(?:_\d+)*\d*(?:\.\d*)?|\.\d\+)(?:e[+-]?\d+)?)[a-z]*/i, a, "0123456789"], ["pln", /^\\[\S\s]?/, a], ["pun", RegExp(e), a]), f(c, d)
            }
            ...
            function h(a, b, c) {
                ...
                for (var b = Math.max(0, b - 1 | 0) || 0, k = 0, m = j.length; m > k; ++k) i = j[k], i.className = "L" + (k + b) % 10, i.firstChild || i.appendChild(h.createTextNode("聽")), l.appendChild(i);
                a.appendChild(l)
            }
...
Namespace.separator = ".", Namespace("com.zybuluo"), com.zybuluo.indexedDB = function () {
    var a = "zybuluoIndexedDB",
        ...
        g = null,
        h = function (d) {
            if (!window.indexedDB) return d(), window.alert("鎮ㄧ殑娴忚鍣ㄤ笉鏀寔 IndexedDB 绂荤嚎瀛樺偍鎶€鏈紝灏嗗奖鍝嶉儴鍒嗙绾垮姛鑳姐€傛偍鍙互鍗囩骇褰撳墠娴忚鍣紝鎴栬€呬娇鐢ㄦ渶鏂扮増鐨� Chrome 娴忚鍣紝鎴栬€呬笅杞戒娇鐢� Cmd Markdown 鐨勫鎴风鐗堟湰銆�"), !1;
    ...
    function a(a, h, i, j, k, l) {
        g !== !0 && (g = !0, e.html(a), f.children("p").html(h), b.modal({
            keyboard: !0
        }), i ? c.data("operation", i) : c.removeData("operation"), j ? d.data("cancelOperation", j) : d.removeData("cancelOperation"), b.unbind("shown"), k && b.on("shown", k), b.unbind("hidden"), b.on("hidden", function () {
            c.removeData("operation"), d.removeData("cancelOperation"), g = !1, l && l(), e.html(""), f.html("<p></p>"), c.html("纭"), d.html("鍙栨秷")
        }))
    }
    ...
    $("a#email-request-verification").click(function (c) {
        c.preventDefault(), $.post($("a#email-request-verification").attr("href"), null, function (c) {
            "sent" == c.success ? $("#resend-confirmation").html(b ? "楠岃瘉閾炬帴宸茬粡鍙戦€佸埌鎮ㄧ殑閭锛� " + c.user_email + ' 璇锋敞鎰忔煡鏀讹紝<a href="' + b + '">杩斿洖棣栭〉</a>銆�' : "楠岃瘉閾炬帴宸茬粡鍙戦€佸埌鎮ㄧ殑閭锛� " + c.user_email + " 璇锋敞鎰忔煡鏀躲€�") : "no_email" == c.fail ? $("#resend-confirmation").html(b ? '鍙戦€侀獙璇侀摼鎺ュけ璐ワ紝<a href="' + b + '">杩斿洖棣栭〉</a>銆�' : "鍙戦€侀獙璇侀摼鎺ュけ璐ャ€�") : c.loggedin === !1 && (window.location = a)
        }, "json")
    })
    ...
    $.ajaxSetup({
        cache: !1
    }), $("input.autofocus").focus(), $("input, textarea").placeholder(), $("div[rel=tooltip]").tooltip()
}), $(function () {
